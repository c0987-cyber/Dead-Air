...
## Adding an answer mid-round

If someone says something that isn't listed but clearly deserves credit, hit
**+ That answer was good — add it** at the bottom of the board. Type it, pick which
rank it slots into, and it drops onto the board marked with a `+`.

Points get interpolated from its neighbours, anything below it shifts down, and any
pick you'd already marked follows its answer down with it. Added answers only live for
that round — the prompt bank is never touched.

Switch it off in settings if you want the board to be the board.

## Game overview

The final screen has a **Game overview** button. It shows which categories came up and
how many times, rounds won, board points, how often each player nailed the actual top
answer, pushes, and how many answers you added. Below that is every round: the category,
the prompt, what each player said, what rank it landed on, and who took it.
...
**Keep the spread flat.** This is the one rule that matters. If a prompt has a single
overwhelming answer, both players say it and the round is a push — the game stalls.
Every top answer in the bank sits between 24 and 34 so there's a real fight for rank.
"Name something you plug in every day" is a bad prompt: everyone says phone. "Name
something you own that's held together with tape" is a good one: six plausible answers,
no runaway favourite.

Ship six answers per prompt so the *Answers shown on the board* setting works at every
value, and keep prompts short — long ones are impossible to lip-read.
